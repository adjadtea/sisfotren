...
class M_data_sekolah extends CI_Model{
	...
	/* by_jenis */
	public function jml_by_jenis($jenis_id){
		$sql = 'SELECT COUNT(id) AS jml FROM '.$this->table_name.' WHERE jenis_id=?';
		$rs = $this->db->query($sql,array($jenis_id));
		$data = $rs->row_array();
		$rs->free_result();
		return $data['jml'];
	}
	public function list_by_jenis($jenis_id){
		if($this->jml_by_jenis($jenis_id) < 1) return '';
