...
class M_santri_file extends CI_Model{
	private $table_name;
	public function __construct(){
		parent::__construct();
		$this->table_name = 'santri_file';
	}
	...
	//by laporan harian
	public function jml_by_santri($santri_id){
		$sql = 'SELECT COUNT(id) AS jml FROM '.$this->table_name.' WHERE santri_id=?';
		$rs = $this->db->query($sql,array($santri_id));
		$data = $rs->row_array();
		$rs->free_result();
		return $data['jml'];
	}
	public function list_nama_by_santri($santri_id){
		if($this->jml_by_santri($santri_id) < 1) return '';
		$sql = 'SELECT id,nama,ukuran,keterangan FROM '.$this->table_name.' WHERE santri_id=?';
		$rs = $this->db->query($sql,array($santri_id));
		$data = $rs->result_array();
		$rs->free_result();
		return $data;
	}
	public function list_by_santri($santri_id){
		if($this->jml_by_santri($santri_id) < 1) return '';
	...
	public function last_by_santri($santri_id){
		if($this->jml_by_santri($santri_id) < 1) return '';
		$this->db->where('santri_id',$santri_id);
