...
class M_data_pegawai_sertifikat_file extends CI_Model{
	private $table_name;
	public function __construct(){
		parent::__construct();
		$this->table_name = 'data_pegawai_sertifikat_file';
	}
	...
	//by laporan harian
	public function jml_by_sertifikat($sertifikat_id){
		$sql = 'SELECT COUNT(id) AS jml FROM '.$this->table_name.' WHERE sertifikat_id=?';
		$rs = $this->db->query($sql,array($sertifikat_id));
		$data = $rs->row_array();
		$rs->free_result();
		return $data['jml'];
	}
	public function list_nama_by_sertifikat($sertifikat_id){
		if($this->jml_by_sertifikat($sertifikat_id) < 1) return '';
		$sql = 'SELECT id,nama,ukuran,keterangan FROM '.$this->table_name.' WHERE sertifikat_id=?';
		$rs = $this->db->query($sql,array($sertifikat_id));
		$data = $rs->result_array();
		$rs->free_result();
		return $data;
	}
	public function list_by_sertifikat($sertifikat_id){
		if($this->jml_by_sertifikat($sertifikat_id) < 1) return '';
		$this->db->where('sertifikat_id',$sertifikat_id);
		$rs = $this->db->get($this->table_name);
		$data = $rs->row_array();
		$rs->free_result();
		return $data;
