...
var <?=$this->router->fetch_class()?> = App.subclass({
	...
	load_data_umum:function(jenis){
		...
		axios.get(this.site_url+'/data_sekolah/'+jenis).then((retval)=>{
			$('#tab_data_umum_'+jenis).html(_.trim(retval.data));
	...
	load_data_kelas:function(id,jenis){
		...
		axios.get(this.jsonParam.app_kelas+'/list_by_tahun_ajaran/'+id,{'jenis':jenis}).then((retval)=>{
			$('#tab_kelas_'+jenis).html(_.trim(retval.data));
	...
	form_add_tahun_ajaran:function(jenis){
		...
		axios.get(this.site_url+'/form_add_tahun_ajaran/'+jenis).then((retval)=>{
			$('#tab_tahun_ajaran_'+jenis).html(_.trim(retval.data));
	...
	form_add_data_umum:function(jenis){
		...
		axios.get(this.site_url+'/form_add_data_sekolah/'+jenis).then((retval)=>{
			$('#isian_data_umum_'+jenis).html(_.trim(retval.data));
	...
	form_edit_data_umum:function(id,jenis){
		$('#isian_data_umum_'+jenis).empty();
		$.app.tunggu('isian_data_umum_'+jenis,1);
		axios.get(this.site_url+'/form_edit_data_sekolah/'+id).then((retval)=>{
			$('#isian_data_umum_'+jenis).html(_.trim(retval.data));
			$.app.tunggu('isian_data_umum_'+jenis,0);
		});
	},
